...
$(document).ready(function () {
    ...
    rangeSlider.noUiSlider.on("update", function (values, event) {
        console.log(values);
        document.getElementById("slider-range-value1").innerHTML = values[0];
        document.getElementById("slider-range-value2").innerHTML = values[1];
        document.getElementsByName("min-price").value = moneyFormat.from(
            values[0]
        );
        document.getElementsByName("max-price").value = moneyFormat.from(
            values[1]
        );
        $("#formFillterPrice").click(function () {
            window.location = UpdateQueryString(
                "max-price",
                document.getElementsByName("max-price").value,
                UpdateQueryString(
                    "min-price",
                    document.getElementsByName("min-price").value
                )
            );
            console.log(
                document.getElementsByName("min-price").value,
